...
void main(){
	...
	generate_code(i, code);
	generate_code(i, code);
	generate_code(i, code);
	decode_code(&recieved_command, code);
	decode_code(&recieved_command, code);
	decode_code(&recieved_command, code);
	decode_code(&recieved_command, code);
	generate_code(i, code);
	generate_code(i, code);
	if (decode_code(&recieved_command, code) == 1){
		if (recieved_command != (uint8_t)i)
			printf("BAD");
			//printf("decrypted sucessfully :) %d\n", recieved_command);
	}
	else{
		printf("no good :(\n");		
	}
